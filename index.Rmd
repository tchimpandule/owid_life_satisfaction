---
title: "Self-reported Happiness, 2014 to 2022"
output: 
  html_document:
    theme: united
---
...
```{r}
cantril %>% 
  mutate(`Cantril ladder score` = round(`Happiness: Happy (aggregate)`, 2)) %>%
  plot_geo(width = 940) %>% 
  add_trace(
...
cantril_chart <- 
  cantril %>% 
  filter(Country %in% c("Norway", "Australia", "Germany", "Bolivia", "Colombia", "Bulgaria ")) %>%
